...
const DeleteThresholdCondition = () =>{
  ...
  const [validDisplayRev , setvalidDisplayRev] =  useState(false);
    ...
    useEffect(async () => {
      // await axios
      //   .get(process.env.REACT_APP_IRMS_RISKRULE_ADMINISTRATION +`/MasterThreshold/GetAll`)
    ...
    const btnBack = () => {
      navigate.push(`/deletethresholdrevisionpage`);
    }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        debugger;
        axios
          .put(process.env.REACT_APP_IRMS_RISKRULE_ADMINISTRATION + `/MasterThreshold/UpdateForModifyStatus?id=${state.id}&code=${99}`)
          .then((res) => {
            debugger;
            console.log(res.data);
    ...
    return(
      ...
      <div className="row">
        <div className="col-md-10 col-lg-10">
          <div hidden={validCondtion} >
          <section id="card">
            <div className="well">
              <h1 style={{ marginTop: "0rem" }}>
                Delete Threshold Condition{" "}
              </h1>
          ...
          <div className="well">
            <div className="warring_card">
              <h2>NOTE</h2>
              <p>There is no Threshold Master in a live status that can be deleted</p>
            </div>
            <br />
            <div className="button">
                <button
          ...
          <tbody>
            {todo2.map((item, index) => (
              <tr>
                ...
                <th scope="col">{item.fromScore}</th>
                <th hidden scope="col">{item.toScoreCondition}</th>
                <th scope="col">{item.toScoreCondition}</th>
                <th scope="col">{item.toScore}</th>
                <th scope="col">{item.caseTypeRef}</th>
...
export default DeleteThresholdCondition